...
Kamei et al. used the 'Just-in Time Quality Assurance' concept, which focuses in identifying defect-prone at level of software changes/patches instead of files or packages. Allowing developers can review these risky changes while they are still fresh in their minds. In the paper, they present a change risk model in which several characteristics of a software changes are implemented(lines added, developer experience...). They empirically evaluated their approach to identify in real time software changes that would be a defect ~\cite{kamei2013large}. Altman et al. used the same concept to carried out a study where the authors applied change classification on a proprietary code case, sharing the experience and lesson learned. They concluded that interpretable prediction models are needed for software defect prediction~\cite{altman1968financial}. 
...
\begin{enumerate}
    ...
    \item Each time that the title or the description of a ticket describes an unexpected behaviour in the program, our criteria indicated that it was considered as a bug report. 
...
\begin{itemize}
    ...
    \item These commits that were committed after the bug report was opened in Launchpad, according to \'Sliwerski et al.~\cite{sliwerski2005changes}.
\end{itemize}
...
\begin{itemize}
  ...
  \item Gramatical bugs influenced from its previous commit. 
\end{itemize}
